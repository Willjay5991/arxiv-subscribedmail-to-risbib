# arxiv-subscribedmail-to-risbib

使用python脚本，提取arxiv订阅邮件中每个论文的条目信息，转为方便管理的RIS或者Bibtex格式。



## 需求:

订阅了arxiv的论文推送提醒，每天都会收到最新论文的推送邮件，但是邮件都是文本格式的，不方便导入论文管理软件（endnote等），使用python脚本，提取arxiv推送邮件中每个论文的条目信息，转为方便管理的RIS或者Bibtex格式。
...
## **举例**
手动将邮件内容复制到txt文件中，运行脚本、处理后的txt文本内容如下：
